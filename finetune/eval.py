...
# Dataset-selectable like train_modal.py. The dataset is passed as a FUNCTION ARGUMENT
# into the remote container (an env var would NOT cross the local->remote boundary — that
# bug silently evaluated CORD once). FF_FT_DATASET is only the local entrypoint default.
_ADAPTERS = {
...
def _hub_adapter(dataset: str) -> str:
    return _ADAPTERS.get(dataset, f"Aarya2004/minicpmv-{dataset}-lora")


def _local_adapter_dir(dataset: str) -> str:
    return f"/cache/ft-out/minicpmv-{dataset}"


def _test_manifest(dataset: str) -> str:
    return f"/cache/{dataset}/test.jsonl"


def _results_path(dataset: str) -> str:
    return f"/cache/{dataset}/results.json"


def _label(dataset: str) -> str:
    return _LABELS.get(dataset, dataset)
...
def evaluate(
    ...
    # Resolve dataset-keyed paths from the ARGUMENT (serialized into this container).
    adapter = adapter or _hub_adapter(dataset)
    test_manifest = _test_manifest(dataset)
    results_path = _results_path(dataset)
    label = _label(dataset)
    local_adapter = _local_adapter_dir(dataset)
    print(f"[eval] dataset={dataset} test={test_manifest} default_adapter={adapter}")
    ...
    if baseline_only:
        ...
        vol.commit()
        print(f"\n=== {label} line-item extraction: baseline only ===")
    ...
    # If the caller didn't override --adapter (so it resolved to the Hub default) and a
    # LOCAL (unpushed) adapter exists on the volume, prefer it — gate-then-publish path.
    if adapter == _hub_adapter(dataset) and os.path.exists(
        os.path.join(local_adapter, "adapter_config.json")
    ):
    ...
    print(f"\n=== {label} line-item extraction: baseline vs tuned ===")
...
def main(
    ...
    """Eval on `--dataset cord` (default) or `--dataset synth`. dataset is passed as an
    ARGUMENT into the container (env vars don't cross). FF_FT_DATASET is the local default."""
    dataset = dataset or os.environ.get("FF_FT_DATASET", "cord")
    print(f"[main] evaluating dataset={dataset}")
    evaluate.remote(dataset=dataset, adapter=adapter, limit=limit, baseline_only=baseline_only)
