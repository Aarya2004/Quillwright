...
*Build Small Hackathon (Gradio × Hugging Face) — June 5–15, 2026*
*Date: 2026-06-05 · Track: Backyard AI (with a dash of delight)*
*Companion docs: [`CONTEXT.md`](../../../CONTEXT.md) (glossary) · [`docs/adr/`](../../adr/) (ADRs 0001–0007)*
...
- A multi-hundred-million-dollar market (ServiceTitan, Jobber, Housecall Pro, Spectora) sells on *"spend less time on paperwork"*; Jobber claims **~7 hrs/week** saved.
...
| "AI features half-baked" | The agentic AI *is* the product |
...
- **Sponsor-aligned:** every anchor sponsor is racing toward on-device, multimodal, *agentic* small models.
...
Capture (photos + voice) → the supervised agent builds a **correct itemized Estimate live** (Facts-from-Tools) → **one Agent Pause + one human Interrupt** → inline-editable Estimate → **PDF export**. **Private Stack only (single Mode).** This *is* the hero demo.
...
### 2.3 Non-goals (YAGNI)
No scheduling/dispatch/payments/CRM/accounting integration. No accounts/multi-user/cloud sync. Not a system of record. Not a diagnostic/safety device. No LLM-invented numbers (ADR-0004).

---

## 3. The model orchestra (verified, ≤32B)

Two **Modes** differ *only* by which model fills each **Model Role** (endpoint swap — ADR-0001, renamed by ADR-0005). Both run on Modal.
...
| Model Role | Private Stack | Best Stack | Modality | Sponsor |
|---|---|---|---|---|
| **Perception** | MiniCPM-V 4.6 (1.3B) | NVIDIA Nemotron 3 Nano Omni | image → text | OpenBMB / NVIDIA |
| **Audio** | whisper-class local STT | Nemotron Omni (native audio) | audio → text | — / NVIDIA |
| **Agent Brain** | gpt-oss-20b | gpt-oss-20b (higher reasoning effort) | text + tool-calling | OpenAI |
| **Multilingual** | Aya Expanse (8B) | Aya 32B / Command | text → text | Cohere |
| **Embedding** | local bge-small class | Cohere Embed | text → vector | — / Cohere |
| **Visual** *(stretch)* | — | Black Forest Labs FLUX | text → image | BFL |

Tool-calling-tuned models de-risk the agent loop (gpt-oss agentic post-training; MiniCPM4-MCP/MiniCPM5 speak MCP/BFCL-style calls).
...
(typed) ┘                        (gpt-oss) ├ lookup_price / compute            └ language toggle (Aya)
...
- **`agent/`** — the Agent Brain (gpt-oss): the plan→act→self-check loop, tool registry, human-in-the-loop checkpoints, and **Trace** emission. Re-derives its next step from current shared run-state each iteration (ADR-0002).
...
- **Interrupt:** human edits an Observation/Line Item anytime → written to shared state → picked up at the agent's *next step* (no restart, no mid-step preemption). Short steps keep it feeling seamless.
...
- **Hero narrative:** *"it does my dreaded paperwork"* (the doer). Other features are supporting beats.
...
OpenAI (gpt-oss Agent Brain) · OpenBMB (MiniCPM-V perception, $10k special category) · NVIDIA (Nemotron Omni; RTX prize) · Cohere (Aya multilingual + Cohere Embed; Toronto) · Modal (hosting/compute credits) · Black Forest Labs (stretch visuals).
...
4. Agent loop (core): gpt-oss plan→act→self-check over `perceive`/`lookup_price`/`compute`/`draft_line_item`/`flag_for_human`; Facts-from-Tools; Trace emission.
