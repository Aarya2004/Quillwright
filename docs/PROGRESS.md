...
_Last updated: 2026-06-13. Branch: `design/fieldforge` (project renamed to Quillwright; git BRANCH name unchanged). **HF Space NOT re-uploaded since the 2026-06-08-early stub** — the live Space is stale (no redesign/pages/chat/real-models). The 2026-06-09/10 model wiring (Modal Best-Stack brain, semantic Recall, spoken Audio) needs torch and is LOCAL-DEMO only — not in the stub Space._

> **Updated 2026-06-13 — Modal deploy status corrected + Finalize & Send shipped:**
>
> - **All four Modal serving apps are now DEPLOYED** (confirmed via `modal app list`, 2026-06-13): `quillwright-brain` (Nemotron 30B-A3B), `quillwright-omni` (S4 Perception+Audio), `quillwright-aya` (multilingual), `quillwright-parse` (S5). Earlier notes below saying "NEITHER app deployed" / "Parse Modal app still NOT deployed" / S4 "Remaining = deploy" are **superseded** — deploy is done. All four sit at **`tasks=0` (scaled to zero)**: cost-safe, but the **first request to each pays a cold-start** (~minutes for the 30B) — warm them before a live demo.
> - **⚠️ NOT yet verified: the HF Space wired to Modal with all models serving at once.** The apps are deployed and the Space-side wiring (`FF_BACKEND`/`FF_MODAL_*_URL` Space secrets) exists, but a full live end-to-end pass on the hosted Space — photo + voice + brain + multilingual all hitting Modal in one sitting — has **not** been run. User will do this live verify. Also unverified: the REAL Parse output path (real table markdown), and Omni serving correctly post-cold-start.
> - **S10 Finalize & Send shipped + pushed** (`22a719d`→`1d1dabd`): SMS (Twilio MMS) + Email (SendGrid, PDF attached) real send on the local path; honest "Draft ready" card on the public Space (no false send). Env-gated (`FF_SEND_ENABLED=1` + creds). Tokenized `/api/estimate_pdf/{token}` for MMS. 184 tests green. **S12 inbound voice-call** still deferred (high-priority stretch).
> - **Fine-tune track** (other Claude instance, `finetune/` only): CORD baseline→tuned shipped, grounded-synthetic generator + dataset-selectable pipeline (`e3387aa`→`165ed67`). `quillwright-ft-train`/`quillwright-ft-eval` Modal apps are theirs.
...
2. **S3 / S4 / S5 — sponsor-model block (equal priority):**
   ...
   - **S4 Nemotron Omni** as selectable Best-Stack Perception. 🟢 **DEPLOYED (2026-06-13):** `modal_omni_app.py` + resolver/client wiring for Perception AND Audio (one deployment, per-role `FF_MODAL_OMNI_URL` opt-in); `quillwright-omni` confirmed deployed via `modal app list`. **Remaining = full live verify on the HF Space** (all models at once, post-cold-start) — user to run.
